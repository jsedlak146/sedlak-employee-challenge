...
function addIntern() { ///you need to replace the engineer stuff with intern stuff
    return inquirer.prompt(question.intern)
    .then((answers) => {
...
function addEngineer() {
    return inquirer.prompt(question.engineer)
    .then((answers) => {
...
function buildTeam() {
    ...
    fs.writeFileSync("./dists/team.html", `
    <!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<body>
    ...
    for (let i = 0; i < team.length; i++){
        fs.appendFileSync("./dists/team.html", `
        <div>${team[i].name}</div>
        <div>${team[i].id}</div>
...
function inti() {
    inquirer.prompt(question.manager)
        .then((answers) => {
